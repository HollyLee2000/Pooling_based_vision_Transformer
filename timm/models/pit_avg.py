...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("送入transformer的x形状: ", x.shape)
        ...
        if self.pool is not None:
            # print("有pool, 输入形状: ", x.shape)
            x = self.pool(x)
            # print("输出形状: ", x.shape)
        # print("pool之后的x.shape: ", x.shape)
        # exit(0)
        return x
...
class ConvHeadPooling(nn.Module):
    ...
    def forward(self, x):
        # print("即将下采样时的形状是: ", x.shape)
        # exit(0)
        x = self.conv(x)
        # print("下采样过后的形状是: ", x.shape)
...
class PoolingVisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        # print("patch_embed之前的形状: ", x.shape)
        x = self.patch_embed(x)
        # print("patch_embed之后的形状: ", x.shape)
        x = self.pos_drop(x + self.pos_embed)
        # cls_tokens = self.cls_token.expand(x.shape[0], -1, -1)
        x = self.transformers(x)
        # cls_tokens = self.norm(cls_tokens)
        # print("transformers出来之后的形状: ", x.shape)
        # exit(0)
        return x
