...
def get_train_logger(logdir: str, filename: str, mode="w") -> Tuple[logging.Logger, str]:  # ->是python函数返回值的注释
    train_log_dir = os.path.join(logdir, "train-logs")
    os.makedirs(train_log_dir, exist_ok=True)  # exist_ok：只有在目录不存在时创建目录，目录已存在时不会抛出异常
    logger_fp = os.path.join(
        train_log_dir,
        "training-" + filename + ".log"
    )
    logger = get_root_logger(
        level=logging.INFO,  # 日志等级为20
        mode=mode,
        name=None,
        logger_fp=logger_fp
    )
    logger.propagate = False
    return logger, logger_fp  # 返回train阶段的logger以及它的名字
...
def get_eval_logger(logdir: str) -> Tuple[logging.Logger, str]:
    ...
    return logger, logger_fp  # 返回eval阶段的logger以及它的名字
